...
function TrackingWhyMatters({ isMobile }: { isMobile: boolean }) {
  const { t } = useTranslation();
  const benefits = t("learnMorePage.trackingBenefits", {
    returnObjects: true,
  }) as string[];
  ...
  return (
    <div className="min-h-screen flex items-center justify-center px-4 py-16">
      <div className="max-w-6xl mx-auto">
        ...
        <TrackingWhyMatters isMobile={isMobile} />
      </div>
    </div>
  );
}

function CSRDSection() {
  const { t } = useTranslation();
    ...
    <div className="min-h-screen flex items-center justify-center px-4 py-16 border-t border-white/10">
      <div className="max-w-6xl mx-auto">
        ...
        <div className="grid md:grid-cols-2 gap-8">
          ...
          />
          <SectionCard
            icon={<Scale className="w-12 h-12 text-blue-3 mb-6 drop-shadow" />}
