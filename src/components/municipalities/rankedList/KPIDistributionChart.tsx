...
function HistogramTooltip({ active, payload, unit }: any) {
  ...
  return (
    <div className="bg-black-2 border border-white/10 rounded-lg px-3 py-2 text-xs shadow-xl">
      <p className="text-white/70 mb-1">
        {Number(bin.min).toFixed(1)}{unit} – {Number(bin.max).toFixed(1)}{unit}
      </p>
      <p className="text-white font-semibold">
        {bin.count} {t("header.municipalities").toLowerCase()}
      </p>
    </div>
  );
}
...
export function KPIDistributionChart({
  ...
  const values = useMemo(
    ...
    [municipalityData, selectedKPI.key],
  );

  const booleanValues = useMemo(() => {
    if (!selectedKPI.isBoolean) return null;
    const trueCount = municipalityData.filter(
      (m) => m[selectedKPI.key] === true,
    ).length;
    const falseCount = municipalityData.filter(
      (m) => m[selectedKPI.key] === false,
    ).length;
    return [
      {
  ...
  }, [municipalityData, selectedKPI, t]);
              ...
              content={({ active, payload }) => {
                ...
                return (
                  <div className="bg-black-2 border border-white/10 rounded-lg px-3 py-2 text-xs shadow-xl">
                    ...
                    <p className="text-white/70">{item.value} kommuner ({pct}%)</p>
                  </div>
                );
              }}
            />
    ...
    <div>
      ...
      <ResponsiveContainer width="100%" height={140}>
        <BarChart data={bins} margin={{ top: 4, right: 4, bottom: 4, left: 4 }}>
          ...
          <Tooltip
            content={(props) => <HistogramTooltip {...props} unit={unit} />}
