...
export function ComparisonEntityHeader({
  ...
  if (dense) {
    return (
      <div className="flex flex-col gap-2 p-3 bg-black-1/40 rounded-level-2 h-full">
        <div className="flex items-start gap-2 min-w-0">
          {item.logoUrl &&
            (isCompany ? (
              <CompanyLogo
                src={item.logoUrl}
                className="shrink-0 rounded-lg size-8 object-contain"
              />
            ) : (
              <img
                src={item.logoUrl}
          ...
          <div className="min-w-0">
            <h3 className="text-base font-light leading-tight line-clamp-2">
              {item.name}
            </h3>
            <p className="text-grey text-xs mt-0.5 line-clamp-2">
              {item.description}
            </p>
          </div>
        </div>
        <LocalizedLink
          to={item.linkTo}
          className="inline-flex items-center gap-1 text-xs text-blue-2 mt-auto"
        >
