-- Secondary query to sum turnover from companies with no reported emissions in 2024
-- This query uses the same logic as companies_no_emissions_2024_with_turnover.sql
-- but returns aggregated totals instead of individual company records
...
companies_no_emissions_2024 AS (
  ...
  FROM pivoted
  WHERE 
    -- Filter for companies with no emissions data in 2024 (both stated and summed totals are 0)
    ...
    -- Only include companies with non-null, non-zero turnover in SEK
    AND turnover_2024 IS NOT NULL 
    AND turnover_2024 > 0
    AND turnover_currency_2024 = 'SEK'
)
SELECT
  COUNT(*) AS total_companies_no_emissions_2024,
  SUM(turnover_2024) AS total_turnover_no_emissions_2024_sek,
  -- Show turnover range for context
  MIN(turnover_2024) AS min_turnover_sek,
  MAX(turnover_2024) AS max_turnover_sek,
  AVG(turnover_2024) AS avg_turnover_sek,
  -- Show breakdown by sector
  STRING_AGG(DISTINCT sector_name, ', ' ORDER BY sector_name) AS sectors_represented
