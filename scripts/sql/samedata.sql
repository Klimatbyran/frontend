...
company_year_signatures AS (
  SELECT
    rwp."companyId",
    rwp."year",
    CONCAT_WS(',',
      CASE WHEN es.s1_total IS NOT NULL THEN 's1_total' END,
      CASE WHEN es.s2_mb IS NOT NULL THEN 's2_mb' END,
      CASE WHEN es.s2_lb IS NOT NULL THEN 's2_lb' END,
      CASE WHEN es.scope3_total IS NOT NULL THEN 'scope3_total' END,
      CASE WHEN es.Scope3_cat_1 IS NOT NULL THEN 'Scope3_cat_1' END,
      CASE WHEN es.Scope3_cat_2 IS NOT NULL THEN 'Scope3_cat_2' END,
      CASE WHEN es.Scope3_cat_3 IS NOT NULL THEN 'Scope3_cat_3' END,
      CASE WHEN es.Scope3_cat_4 IS NOT NULL THEN 'Scope3_cat_4' END,
      CASE WHEN es.Scope3_cat_5 IS NOT NULL THEN 'Scope3_cat_5' END,
      CASE WHEN es.Scope3_cat_6 IS NOT NULL THEN 'Scope3_cat_6' END,
      CASE WHEN es.Scope3_cat_7 IS NOT NULL THEN 'Scope3_cat_7' END,
      CASE WHEN es.Scope3_cat_8 IS NOT NULL THEN 'Scope3_cat_8' END,
      CASE WHEN es.Scope3_cat_9 IS NOT NULL THEN 'Scope3_cat_9' END,
      CASE WHEN es.Scope3_cat_10 IS NOT NULL THEN 'Scope3_cat_10' END,
      CASE WHEN es.Scope3_cat_11 IS NOT NULL THEN 'Scope3_cat_11' END,
      CASE WHEN es.Scope3_cat_12 IS NOT NULL THEN 'Scope3_cat_12' END,
      CASE WHEN es.Scope3_cat_13 IS NOT NULL THEN 'Scope3_cat_13' END,
      CASE WHEN es.Scope3_cat_14 IS NOT NULL THEN 'Scope3_cat_14' END,
      CASE WHEN es.Scope3_cat_15 IS NOT NULL THEN 'Scope3_cat_15' END,
      CASE WHEN es.Scope3_cat_16 IS NOT NULL THEN 'Scope3_cat_16' END
    ) AS signature
  FROM reporting_with_prev rwp
  LEFT JOIN emissions_with_scopes es ON rwp."id" = es."reportingPeriodId"
  WHERE rwp."year" IN ('2023', '2024')
),
matching_companies AS (
  SELECT
    s2023."companyId",
    s2023.signature
  FROM company_year_signatures s2023
  JOIN company_year_signatures s2024
    ON s2023."companyId" = s2024."companyId"
    AND s2023.signature = s2024.signature
    AND s2023."year" = '2023'
    AND s2024."year" = '2024'
),
...
change_rates AS (
  SELECT
    ...
    ce."companyId",
    ce.year,
    -- Total % change using Scope 1 + Scope 2 only
    ROUND(((
      (COALESCE(ce.s1_total,0) + COALESCE(ce.s2_mb,0) + COALESCE(ce.s2_lb,0)) -
      (COALESCE(pe.s1_total,0) + COALESCE(pe.s2_mb,0) + COALESCE(pe.s2_lb,0))
    ) / NULLIF((COALESCE(pe.s1_total,0) + COALESCE(pe.s2_mb,0) + COALESCE(pe.s2_lb,0)), 0))::NUMERIC, 2) AS total_change_pct_s1s2,
    -- Total % change using Scope 1 + Scope 2 + Scope 3 stated
    ROUND(((
      (COALESCE(ce.s1_total,0) + COALESCE(ce.s2_mb,0) + COALESCE(ce.s2_lb,0) + COALESCE(ce.scope3_total,0)) -
      (COALESCE(pe.s1_total,0) + COALESCE(pe.s2_mb,0) + COALESCE(pe.s2_lb,0) + COALESCE(pe.scope3_total,0))
    ) / NULLIF((COALESCE(pe.s1_total,0) + COALESCE(pe.s2_mb,0) + COALESCE(pe.s2_lb,0) + COALESCE(pe.scope3_total,0)), 0))::NUMERIC, 2) AS total_change_pct_stated,
    -- Total % change using Scope 1 + Scope 2 + Scope 3 sum
    ROUND(((
      (COALESCE(ce.s1_total,0) + COALESCE(ce.s2_mb,0) + COALESCE(ce.s2_lb,0) + COALESCE(ce.scope3_categories_sum,0)) -
      (COALESCE(pe.s1_total,0) + COALESCE(pe.s2_mb,0) + COALESCE(pe.s2_lb,0) + COALESCE(pe.scope3_categories_sum,0))
    ) / NULLIF((COALESCE(pe.s1_total,0) + COALESCE(pe.s2_mb,0) + COALESCE(pe.s2_lb,0) + COALESCE(pe.scope3_categories_sum,0)), 0))::NUMERIC, 2) AS total_change_pct_sum,
    -- Combined change rate
    (
      ROUND(
        CAST(
          (
            (
              COALESCE(ce.s1_total,0) + COALESCE(ce.s2_mb,0) + COALESCE(ce.s2_lb,0) + COALESCE(ce.scope3_total,0)
              + COALESCE(ce.Scope3_cat_1,0) + COALESCE(ce.Scope3_cat_2,0) + COALESCE(ce.Scope3_cat_3,0)
              + COALESCE(ce.Scope3_cat_4,0) + COALESCE(ce.Scope3_cat_5,0) + COALESCE(ce.Scope3_cat_6,0)
              + COALESCE(ce.Scope3_cat_7,0) + COALESCE(ce.Scope3_cat_8,0) + COALESCE(ce.Scope3_cat_9,0)
              + COALESCE(ce.Scope3_cat_10,0) + COALESCE(ce.Scope3_cat_11,0) + COALESCE(ce.Scope3_cat_12,0)
              + COALESCE(ce.Scope3_cat_13,0) + COALESCE(ce.Scope3_cat_14,0) + COALESCE(ce.Scope3_cat_15,0)
              + COALESCE(ce.Scope3_cat_16,0)
            ) -
            (
              COALESCE(pe.s1_total,0) + COALESCE(pe.s2_mb,0) + COALESCE(pe.s2_lb,0) + COALESCE(pe.scope3_total,0)
              + COALESCE(pe.Scope3_cat_1,0) + COALESCE(pe.Scope3_cat_2,0) + COALESCE(pe.Scope3_cat_3,0)
              + COALESCE(pe.Scope3_cat_4,0) + COALESCE(pe.Scope3_cat_5,0) + COALESCE(pe.Scope3_cat_6,0)
              + COALESCE(pe.Scope3_cat_7,0) + COALESCE(pe.Scope3_cat_8,0) + COALESCE(pe.Scope3_cat_9,0)
              + COALESCE(pe.Scope3_cat_10,0) + COALESCE(pe.Scope3_cat_11,0) + COALESCE(pe.Scope3_cat_12,0)
              + COALESCE(pe.Scope3_cat_13,0) + COALESCE(pe.Scope3_cat_14,0) + COALESCE(pe.Scope3_cat_15,0)
              + COALESCE(pe.Scope3_cat_16,0)
            )
          ) / NULLIF(
            (
              COALESCE(pe.s1_total,0) + COALESCE(pe.s2_mb,0) + COALESCE(pe.s2_lb,0) + COALESCE(pe.scope3_total,0)
    ...
    ) AS combined_change_rate,
    -- Calculate % change for each field
...
SELECT
  c."name" AS "Company",
  cr.year AS "Year",
  cr.total_change_pct_s1s2,
  cr.total_change_pct_stated,
  cr.total_change_pct_sum,
  cr.combined_change_rate,
  -- Scope 1
  ce.s1_total,
...
INNER JOIN matching_companies mc ON ce."companyId" = mc."companyId"
