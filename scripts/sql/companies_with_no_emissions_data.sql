-- Query to find companies with no reported emissions data across all years
-- Returns: company name, sector, industry name, revenue
-- Filters for companies with 0 or null emissions data for all years
...
company_emissions_summary AS (
  SELECT
    ...
    ig."sectorName",
    ig."industryName",
    -- Check if company has any non-null, non-zero emissions data across all years
    MAX(CASE 
      WHEN rwc.s1_total IS NOT NULL AND rwc.s1_total > 0 THEN 1 
      ELSE 0 
    END) AS has_scope1_data,
    MAX(CASE 
      WHEN (rwc.s2_mb IS NOT NULL AND rwc.s2_mb > 0) OR (rwc.s2_lb IS NOT NULL AND rwc.s2_lb > 0) THEN 1 
      ELSE 0 
    END) AS has_scope2_data,
    MAX(CASE 
      WHEN rwc.scope3_total IS NOT NULL AND rwc.scope3_total > 0 THEN 1 
      ELSE 0 
    END) AS has_scope3_stated_data,
    MAX(CASE 
      WHEN rwc.scope3_categories_sum IS NOT NULL AND rwc.scope3_categories_sum > 0 THEN 1 
      ELSE 0 
    END) AS has_scope3_categories_data,
...
SELECT
  ...
  revenue,
  revenue_currency
FROM company_emissions_summary
WHERE 
  -- Filter for companies with no emissions data across all scopes and all years
  has_scope1_data = 0 
  AND has_scope2_data = 0 
  AND has_scope3_stated_data = 0 
  AND has_scope3_categories_data = 0
