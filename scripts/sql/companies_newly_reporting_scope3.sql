...
SELECT
  ...
  (ARRAY(SELECT unnest(scope3_cats_2024)
         EXCEPT
         SELECT unnest(scope3_cats_2023)) <> '{}') AS new_scope3_categories,
  c.tags
FROM pairs
LEFT JOIN "Company" c ON pairs.company_name = c."name"
)
SELECT
  ...
  COUNT(*) FILTER (WHERE has_any_scope3_categories) AS num_scope3_any_categories,
  COUNT(*) FILTER (WHERE new_scope3_categories) AS num_new_scope3_categories,
  COUNT(*) FILTER (WHERE new_scope3_categories AND 'large-cap' = ANY(tags)) AS num_new_scope3_large_cap,
  COUNT(*) FILTER (WHERE new_scope3_categories AND (tags IS NULL OR 'mid-cap' = ANY(tags))) AS num_new_scope3_mid_cap_or_null,
  STRING_AGG(company_name, ', ' ORDER BY company_name) FILTER (WHERE new_scope3_categories) AS companies_new_scope3,
  STRING_AGG(company_name, ', ' ORDER BY company_name) FILTER (WHERE new_scope3_categories AND 'large-cap' = ANY(tags)) AS companies_new_scope3_large_cap,
  STRING_AGG(company_name, ', ' ORDER BY company_name) FILTER (WHERE new_scope3_categories AND (tags IS NULL OR 'mid-cap' = ANY(tags))) AS companies_new_scope3_mid_cap_or_null
FROM final; 