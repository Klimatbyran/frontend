-- Query to get 2024 data with revenue intensity for companies with non-null, non-zero SEK turnover
-- Filters out companies without turnover data or with non-SEK currency
-- Includes both regular intensity and per million currency intensity calculations
...
SELECT
  ...
  -- revenue intensity 2024 per million SEK (emissions per million SEK)
  CASE 
    WHEN revenue_2024 IS NOT NULL AND revenue_2024 > 0 
    THEN ROUND(
    ...
    ELSE NULL 
  END AS revenue_intensity_per_million_sek_2024_stated,
  CASE 
    WHEN revenue_2024 IS NOT NULL AND revenue_2024 > 0 
    THEN ROUND(
      ((COALESCE(s1_2024,0) + COALESCE(NULLIF(s2_mb_2024,0), s2_lb_2024, 0) + COALESCE(scope3_sum_2024,0)) / (revenue_2024 / 1000000))::numeric, 
      6
    )
    ELSE NULL 
  END AS revenue_intensity_per_million_sek_2024_summed
FROM pivoted
WHERE 
