-- Query to find all companies with split reporting years
-- Returns companies where their latest reporting period has different start and end years
-- Shows turnover, currency, emissions data, and reporting period details
...
pivoted AS (
  SELECT
    ...
    -- Get the latest reporting year based on endDate
    (SELECT EXTRACT(YEAR FROM "endDate") FROM company_years cy2 
     WHERE cy2."wikidataId" = company_years."wikidataId" 
     AND cy2."endDate" IS NOT NULL
     ORDER BY cy2."endDate" DESC LIMIT 1) AS latest_reporting_year
  FROM company_years
...
SELECT
  ...
  sector_name,
  industry_name,
  -- latest reporting period details
  latest_reporting_year,
  latest_start_date,
  latest_end_date,
  -- show the year difference
  CASE 
    WHEN latest_start_date IS NOT NULL AND latest_end_date IS NOT NULL 
    THEN EXTRACT(YEAR FROM latest_end_date) - EXTRACT(YEAR FROM latest_start_date)
    ELSE NULL
  END AS year_span,
  -- 2024 emissions data (if available)
  ...
  -- turnover data (if available)
  turnover_2024,
  turnover_currency_2024
FROM pivoted
WHERE 
  -- Filter for companies with split reporting years
  latest_start_date IS NOT NULL 
  AND latest_end_date IS NOT NULL 
  AND EXTRACT(YEAR FROM latest_start_date) != EXTRACT(YEAR FROM latest_end_date)
-- order by year span (longest first), then by sector name, then by company name
ORDER BY 
  year_span DESC,
  sector_name, 
  company_name;
